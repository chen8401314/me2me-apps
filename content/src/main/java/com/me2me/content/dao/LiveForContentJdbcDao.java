...
//    	sb.append(" limit ").append(start).append(",").append(pageSize);
    ...
    public List<Map<String, Object>> getTagTopicInfo(List<Long> tagIds){
    	...
    	StringBuilder sb = new StringBuilder();
    	sb.append(" select o1.tag_id,SUM(o1.topic_count) kingdomCount,SUM(o1.price_sum) tagPrice,o2.uid_count tagPersons from ( ");
    	sb.append(" select t4.tag_id tag_id, COUNT(distinct t1.id) topic_count,SUM(distinct t1.price) price_sum");
    	sb.append(" from topic t1, content t2, topic_tag_detail t3,(select id,tag,IF(id in (");
        sb.append(tagIdsStr);
    	sb.append(")");
    	sb.append(" ,id,pid) tag_id from topic_tag where id  in (");
        sb.append(tagIdsStr);
    	sb.append(" ) or pid in (");
        sb.append(tagIdsStr);
    	sb.append(" )) t4");
    	sb.append(" where t1.id = t2.forward_cid  and t4.id = t3.tag_id and t2.type = 3 and t1.id = t3.topic_id ");
    	sb.append(" and t1.status = 0 and t3.status = 0 group by t4.tag_id) o1, ");
    	sb.append(" (select m.tag_id,COUNT(m.uid)+FLOOR(SUM(m.read_count)/20) uid_count from(select distinct t4.tag_id tag_id,t1.uid,t3.read_count");
    	sb.append("  from topic_fragment t1, topic_tag_detail t2, content t3,(select id,tag,IF(id in (");
        sb.append(tagIdsStr);
    	sb.append("),id,pid) tag_id from topic_tag where id  in (");
        sb.append(tagIdsStr);
    	sb.append(") or pid in (");
        sb.append(tagIdsStr);
    	sb.append(")) t4 ");
    	sb.append(" where  t4.id = t2.tag_id and t1.topic_id = t2.topic_id");
    	sb.append(" and t3.type = 3 and t1.topic_id = t3.forward_cid and t2.status=0 ) m group by tag_id ) o2");
